  ...
  $('[data-ca-form-group="processor"] select').each(function (i, elm) {
    if (!$(elm).data('ca-processor-procesed')) {
      ...
      $(elm).change(function (e) {
        fn_paypal_perform_redesign(app, $(elm).closest('[data-ca-form-group="main"]'), $(elm).val());
      });
    }
  });
}
...
function fn_paypal_get_redesign_scheme(app, direction) {
  if (direction == 'paypal') {
    return {
      rearrange: ['processor', 'name', 'company', 'tpl', 'category', 'icon', 'status', 'description', 'instructions', 'usergroup', 'taxes', 'surcharge', 'surcharge_title'],
      callbacks: [function (container) {
        // rename labels
        var dst;
        ...
        // set payment name as a processor name
        ...
        // set paypal icon as a default icon
    ...
    return {
      rearrange: ['name', 'company', 'processor', 'tpl', 'category', 'usergroup', 'description', 'update_divider', 'surcharge', 'surcharge_title', 'taxes', 'instructions', 'status', 'icon'],
      callbacks: [function (container) {
        // rename labels
        var dst;
        dst = $('[data-ca-form-group="description"] label', container);
