    ...
    {capture name="tags_table"}
        <div class="table-responsive-wrapper longtap-selection">
            ...
            <tr>
                ...
                <th width="40%"><a class="cm-ajax{if $search.sort_by == "tag"} sort-link-{$search.sort_order_rev}{/if}" href="{"`$c_url`&sort_by=tag&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("tag")}</a></th>
                {foreach from=$tag_objects item="o"}
                <th width="8%" class="center">&nbsp;&nbsp;{__($o.name)}&nbsp;&nbsp;</th>
                {/foreach}
                <th width="8%">&nbsp;</th>
                <th class="right" width="10%"><a class="cm-ajax{if $search.sort_by == "status"} sort-link-{$search.sort_order_rev}{/if}" href="{"`$c_url`&sort_by=status&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id="pagination_contents">{__("status")}</a></th>
            </tr>
            </thead>
