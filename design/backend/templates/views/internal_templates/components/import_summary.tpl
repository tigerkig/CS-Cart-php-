<div class="import-summary">
    ...
    {else}
        <div class="alert alert-success">
            <p>{__("internal_template_import_success_msg")}</p>
        </div>
    {/if}
    <table width="100%" class="table table-no-hover table--relative">
        <tr class="no-border">
            <td width="60%"><strong>{__('count_internal_template_successfully_imported')}</strong></td>
            <td align="right">{$import_result.count_success_templates}</td>
        </tr>
        <tr>
            <td width="60%"><strong>{__('count_internal_template_fail_imported')}</strong></td>
