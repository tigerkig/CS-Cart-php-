                ...
                {foreach from=$sfs item="sf" name="foreach_sfs"}
                    <li>
                        {if $sf}
                            ...
                            <label class="checkbox checkbox--multiline" for="elm_{$sf.name|md5}"><input type="checkbox" value="Y" name="selected_fields{$sf.name}" id="elm_{$sf.name|md5}" checked="checked" {if $sf.disabled == "Y"}disabled="disabled"{/if} class="cm-item-s" />
                            {$sf.text}</label>
                        {/if}
                    </li>
                {/foreach}
