...
<tr {if !$clone}id="{$holder}_{$order_id}" {/if}class="cm-js-item{if $clone} cm-clone hidden{/if}">
    ...
    <td data-th="{__("status")}">{if $clone}{$status}{else}{include file="common/status.tpl" status=$status display="view" name="order_statuses[`$order_id`]"}{/if}</td>
    <td data-th="{__("customer")}">{$customer}</td>
    <td data-th="{__("date")}">
