    ...
    <td width="600" style="border: #444444; border-style: solid; border-width: 2px" align="center">
        ...
        <tr>
            ...
            {foreach from=$order_info.products item="oi"}
                {if (!empty($oi.extra.supplier_id) && $oi.extra.supplier_id == $supplier.supplier_id) || (fn_get_product_supplier_id($oi.product_id) == $supplier.supplier_id)}
                    <tr>
                        <td bgcolor="#ffffff">{$oi.product_code|default:"-"}</td>
                        <td bgcolor="#ffffff">{$oi.product}
                            {if $oi.product_options}<div style="padding-top: 1px; padding-bottom: 2px;">{include file="common/options_info.tpl" product_options=$oi.product_options|fn_get_selected_product_options_info}</div>{/if}</td>
